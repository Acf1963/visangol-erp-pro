...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  const variants = {
    primary: 'bg-orange-500 text-white hover:bg-orange-400 shadow-lg shadow-orange-500/20 active:scale-95',
    ...
    outline: 'bg-transparent border border-slate-700 text-slate-300 hover:bg-slate-800 active:scale-95',
  };
...
export const Card: React.FC<{ children: React.ReactNode; className?: string; title?: string; icon?: React.ReactNode }> = ({ children, className, title, icon }) => (
  <div className={cn('bg-slate-800/40 border border-slate-700 rounded-3xl backdrop-blur-xl overflow-hidden', className)}>
    {title && (
