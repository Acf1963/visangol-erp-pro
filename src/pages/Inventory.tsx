...
export const Inventory: React.FC = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setIsImporting(true);
      const parsedProducts = await parseExcelToProducts(file);

      if (parsedProducts.length > 0) {
        if (
          window.confirm(
            `Foram encontrados ${parsedProducts.length} produtos no ficheiro Excel. Deseja importar/atualizar o stock?`,
          )
        ) {
        ...
        alert('Nenhum produto válido encontrado no ficheiro. Verifique se o formato está correto.');
      }
    } catch (error) {
      console.error('Erro ao importar Excel:', error);
      alert('Ocorreu um erro ao ler o ficheiro Excel. Verifique se o formato está correto.');
    } finally {
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-xs font-mono uppercase tracking-[0.3em] mt-2">
            Gestão de Produtos e Lubrificantes
          </p>
        </div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex gap-3 mt-1">
                      ...
                      <span className="text-[10px] font-bold text-orange-500 uppercase tracking-widest bg-orange-500/10 px-2 rounded">
                        {product.category}
                      </span>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 w-full md:w-auto">
                  <div className="text-center md:text-right">
                    ...
                    <p
                      className={cn(
                        'text-xl font-black',
                        product.stock_current < (product.min_stock_alert || 5)
                          ? 'text-red-400'
                          : 'text-white',
                      )}
                    >
                      {product.stock_current}{' '}
                      <span className="text-xs opacity-50">{product.unit}</span>
                    </p>
                  </div>
                  <div className="text-center md:text-right">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">
                    ...
                    <p className="text-xl font-black text-orange-400">
                      {product.prices?.pvp1?.toLocaleString() || 0}{' '}
